...
| Peak RSS, ~1 page | 15 MB | 64 MB | 4.2x lower |
| Peak RSS, ~11 pages | 17 MB | 122 MB | 7.3x lower |
| Output size, ~1 page | 10.4 KB | 17.1 KB | inkmd 1.6x smaller |
| Output size, ~11 pages | 150.1 KB | 121.8 KB | WeasyPrint 1.23x smaller |

inkmd is 6 to 10x faster, 4 to 7x lighter on memory, and ~6x smaller to install. Output size goes either way depending on document size (see "Output size crossover" below).
...
- **WeasyPrint can do more.** inkmd v0.1 does not support images, full Unicode (CJK, Cyrillic, etc), task lists, or page-splitting for oversized tables. WeasyPrint handles all of these. This is not a feature-parity comparison; it is a "for the things inkmd does, how does it compare" comparison. Use the right tool for your inputs.
...
Run on 2026-05-12 against `inkmd 0.1.0` and `weasyprint 68.1` with `markdown 3.10.2`, Python 3.13, on SteamOS x86_64.

### Install footprint

```
inkmd venv:      11.6 MB
weasyprint venv: 74.5 MB
```

A complete `inkmd` install is 11.6 MB total venv. A complete `weasyprint + markdown` install is 74.5 MB.

The `inkmd` package itself is 1.2 MB. About 4,700 lines of that is generated AFM kerning data; the rest of the package is around 500 KB. The remainder of the venv is Python's `pip` and base files.
...
Small (~1 page, ~1 KB markdown -> ~10 KB PDF):
  inkmd:      median 112 ms, min 101 ms
  weasyprint: median 741 ms, min 745 ms
  ratio: WeasyPrint takes 6.6x longer

Medium (~11 pages, ~15 KB markdown -> ~150 KB PDF):
  inkmd:      median 148 ms, min 147 ms
  weasyprint: median 1.40 s, min 1.40 s
  ratio: WeasyPrint takes 9.5x longer
```
...
Medium (~11 pages, ~120 KB PDF): inkmd 150.1 KB, weasyprint 121.8 KB (WeasyPrint 1.23x smaller)
```

WeasyPrint compresses its content streams (flate/zlib), which is a fixed-cost saving that scales with PDF body content. inkmd does not compress streams in v0.1; this is queued for a v0.1.x patch and is the obvious next perf win.
...
- **WeasyPrint is the right tool when** you need images, non-Latin scripts, CSS-style control over layout, page chrome, or any of the features inkmd v0.1 doesn't have. WeasyPrint also produces slightly smaller PDFs for longer documents.
