...
- **WeasyPrint can do more.** inkmd supports images (PNG, JPEG) and task lists; Cyrillic, Greek, and Latin-Extended render via an embedded font, but it does not yet support CJK text (the bundled font lacks CJK glyphs). WeasyPrint handles full Unicode out of the box. This is not a feature-parity comparison; it is a "for the things inkmd does, how does it compare" comparison. Use the right tool for your inputs.
