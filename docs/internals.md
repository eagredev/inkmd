...
The complexity profile is recorded in [`LIZARD-AUDIT.md`](../LIZARD-AUDIT.md), re-measured at v0.3: 27 functions exceed the CCN-15 warn threshold. The bulk are spec-driven parsing functions whose cyclomatic complexity reflects how branchy the spec is, not disorder - `_resolve_emphasis` is a faithful translation of CommonMark §6.2, `_try_html_tag` enumerates the §6.6 inline-HTML shapes, and so on. The one function that has grown past comfortable density is `_BlockParser.feed`, now at CCN 145: the block container state machine described above is a single large dispatch (the conformance work added the per-item indent, lazy-continuation, tab, and HTML-block branches inline). It is the primary standing refactor candidate - the natural split is to lift container-prefix consumption, the list-stack walk, and the post-walk content routing into three named phases - and it is held together by the 823-test suite plus the full spec suites rather than by being small. `paginate_runs` (CCN 58) in `layout.py` is the other long-standing candidate. Neither blocks correctness; both are flagged honestly so the next person reading the code knows where the density lives.
