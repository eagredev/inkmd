...
There are several existing ways to turn markdown into PDF. Each has a real audience, and inkmd does not displace any of them; it occupies a niche that the others don't fit. This page covers where it does and does not.
...
[WeasyPrint](https://weasyprint.org/) takes HTML+CSS and produces a PDF. You add a markdown stage in front of it (typically `markdown-it-py` or `python-markdown`) and you have a markdown-to-PDF pipeline.
...
- You pay for two parsers (markdown to HTML to PDF) where you only need one (markdown to PDF). For high-throughput cases this matters.
...
inkmd's bet is that the markdown-to-HTML-to-PDF stack is a historical accident. Markdown is a structured-text format; PDF is a structured-text format; HTML is a different model with its own concerns (responsive layout, JavaScript, accessibility tagging) that the PDF stage has to undo.
...
- Pandoc is GPL, a non-issue for most users, but it blocks some closed-source / commercial use.
...
[PyMuPDF](https://pymupdf.readthedocs.io/) wraps MuPDF, a C PDF library. Various wrappers add markdown-to-PDF on top.
...
If you're already using one of these parsers for markdown-to-HTML elsewhere in your stack and don't need PDF, keep using them. If you're starting fresh and you want PDF, inkmd is direct.
...
3. **Measured CommonMark conformance.** 100% on the public spec suite, with the full breakdown in [`docs/conformance.md`](conformance.md).
4. **MIT-licensed.** Use it anywhere.

If those four matter for what you're building, inkmd fits. If they don't, the alternatives are usually a better choice.
