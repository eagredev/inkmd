...
## How to frame it (positioning)

The conformance percentage (CommonMark 554/652, GFM-ext 20/28 as of v0.2) is
**evidence for the principle, not the principle itself.** When describing
inkmd to users, lead with the principle:

> **Utter consistency — what GitHub showed you is what you get.**

not with "X% on the spec suite." Users understand "it renders your document
the way every other tool does" far better than a benchmark number, and that
sentence is the actual promise. The number is how we *measure* progress toward
it; it isn't the selling point.
...
a broken promise. (This is why inkmd's release tiers — see the roadmap — are
defined by user-visible quality, not by hitting a conformance number.)
...
looking at.** A markdown→PDF renderer's "interface" is the meaning of each
