...
later. Written at decision time so the rationale, including the
still-unproven parts, survives for a future `inkmd-lite` build.
...
| inkmd **zipapp** (`inkmd.pyz`, font-less) | ~170 KB | the existing font-less tier |
...
### 1. The default tier optimises for correct output, not for an install-size number the median user never notices.
...
anything else*. The larger default was acceptable precisely *because*
walking it back is cheap. (See "The reversibility seams" below for the exact
...
| COLRv1 vector emoji instead of CBDT bitmaps | Far harder to implement in a hand-rolled reader; the CBDT-to-PNG-to-XObject path reuses the existing image pipeline. (See `emoji-rendering-plan.md`.) Size is comparable anyway. |
...
2. **The font-absent path is the same as the always-supported fallback
   ...
   applies the `emoji_fallback` policy via a `ContextVar`: `"name"` (the
   default, giving `[rocket]`-style labels) or `"drop"`. This path is fully exercised by
   the test suite (it is the zipapp's behaviour), so font-less is not an
   untested mode; it is a tested tier.
3. **A working font-less artefact already ships.** The zipapp build
   (`scripts/build_zipapp.py`) omits the font with one line,
   `shutil.ignore_patterns("assets")`, and is the ~170 KB `inkmd.pyz`.
...
Whether the lift is easy or hard is only knowable after the extraction is
actually done. The claim here is that the design was cut for reversibility;
that claim is unproven until a real font-less build either confirms it or
exposes where a hidden dependency on the bundled font remains.
