...
- Hand-rolled parsing of the table directory, `cmap` format 12 (codepoint
  → gid), `cmap` format 14 (variation selectors), and the `CBLC`/`CBDT`
...
- **Transparency from `tRNS`:** build a soft mask. For indexed, `tRNS`
  ...
  each source pixel. That requires expanding indexed pixels → alpha, which
...
- `glyph_png(gid) -> (png_bytes, metrics)` via the CBLC strike → index
  subtable (formats 1 & 2) → CBDT (image formats 17 & 18). Cache by gid.
...
- Tests: compile `"# 🚀 Launch"` → PDF contains an image XObject + SMask,
...
- `emoji_font`: parse `GSUB` → script/feature/lookup tables → **type 4
...
- Build a curated subset (~200–400 common README emoji: status, arrows,
  ...
  extracted PNGs + a codepoint→PNG index sidesteps the font-file question
...
- Confirm CommonMark/GFM conformance unchanged (emoji is a render concern,
  not a parse one — should be zero movement).
...
1. **GSUB ligatures** (Phase 4) — skip → component glyphs render separately.
