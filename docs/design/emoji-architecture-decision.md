...
runs anywhere, deterministic.** Drawing a real 🚀 means embedding glyph data
— and a font is exactly the kind of heavyweight asset the project exists to
avoid. The apparent options were all bad: ship `?` (looks broken), do
monochrome only (still need a font, only half-solves it), or take a font
dependency (betrays the promise).

## The reframe that dissolved the tension

The tension dissolved on one observation: **color emoji via bitmaps is the
...
> path that already exists → place it inline scaled to the font size.
...
So color-bitmap was simultaneously the least code, the best-looking default,
and the one that reused machinery inkmd already had and tested.

**Transferable lesson:** when a feature looks like it violates a core
constraint, look for the seam where the "hard" version collapses into
something you already have. The reframe here wasn't cleverness in the emoji
domain — it was noticing that "draw a colored glyph" and "embed a PNG" are
the same operation, and inkmd was already good at the second.

## Decisions

### Fidelity: bitmap color now; vector later only if asked.
CBDT PNG → Image XObject. The documented tradeoff: bitmaps soften when scaled
...
Consistent with inkmd's whole ethos (the PDF layer is hand-written too), the
...
## The "zero dependencies" promise, sharpened

Shipping a 10 MB font asset forced an honest restatement of what "zero
dependencies" actually buys, versus what it was conflated with as a slogan.
The *real* engineering value is: **installs anywhere, no C-extension platform
matrix, no supply-chain surface, deterministic output.** Every one of those
properties survives a bundled pure-data asset. What "zero deps" was *not*
meant to be is a straitjacket against ever shipping data.

So the honest framing is **"zero *required* runtime dependencies"** — no
third-party packages, no system libraries, no network, no C extensions. A
bundled font (or, later, an optional heavyweight feature) doesn't betray that;
it's data, not a dependency. This decoupling is what made the emoji decision
clean rather than a promise-breaking compromise.
